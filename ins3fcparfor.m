...
%subdirs = subdirs(300:min(302, length(subdirs)));
...
for i = 1:length(subdirs)
    ...
    for j = 1:length(zip_files)
        ...
        if ~exist(nii_file, 'file')
            disp(['NIfTI file does not exist for subject: ', new_folder_name, '. Skipping...']);
            continue;
        end
        ...
        BATCH.Analysis.sources = {'Amygdala_L'};
...
% 
% nnz(data)AAL_61x73x61_YCG
% [data,~,~,head]=y_ReadAll('Amygdala_L.nii');
